...
class RtmMessageHandler(
    ...
    private val encryptor: (plain: ByteArray) -> String?,
     ...
     */
    fun sendChatBarrage(text: String, onComplete: (Boolean) -> Unit = {}) {
        if (text.isEmpty()) return
        val json = JSONObject()
            .put(RTM_MESSAGE_KEY_TEXT, text)
            .put(RTM_MESSAGE_KEY_TOPIC, RTM_MESSAGE_TOPIC_CHAT)
            .toString()
        send(topic = RTM_MESSAGE_TOPIC_CHAT, payload = json, encrypt = true, onComplete = onComplete)
    }
    ...
    fun toggleMute(target: Participant) {
        ...
        val msg = RtmMessage(topic = RTM_MESSAGE_TOPIC_MUTE, identities = identities, sendTimestamp = System.currentTimeMillis())
        val json = Json.encodeToString(RtmMessage.serializer(), msg)
        send(topic = RTM_MESSAGE_TOPIC_MUTE, payload = json, encrypt = true, onComplete = {}, identities = identities)
    }
     ...
     */
    fun handleDataReceived(event: io.livekit.android.events.RoomEvent.DataReceived, onChat: (Participant, String) -> Unit, onMuteMe: () -> Unit, onResumeMe: () -> Unit, onEndCall: () -> Unit, onCountDown: (CountDownTimerData, String) -> Unit, onHandsUp: (HandsUpData, String) -> Unit) {
        ...
        when (topic) {
            RTM_MESSAGE_TOPIC_CHAT, RTM_MESSAGE_TOPIC_MUTE, RTM_MESSAGE_TOPIC_RESUME_CALL -> {
                ...
                scope.launch(Dispatchers.IO) {
                    ...
                    when (topic) {
                        RTM_MESSAGE_TOPIC_CHAT -> rtm?.text?.let { onChat(p, it) }
    ...
    fun sendEndCall(onComplete: (Boolean) -> Unit) {
        ...
        send(topic = RTM_MESSAGE_TOPIC_END_CALL, payload = json, encrypt = true, onComplete = onComplete)
    }
    ...
    fun sendContinueCallRtmMessage(participant: Participant) {
        ...
        participant.identity?.let { identity ->
            ...
            send(topic = RTM_MESSAGE_TOPIC_RESUME_CALL, payload = json, encrypt = true, onComplete = {}, identities = identities)
        }
    }
    ...
    fun sendRaiseHandsRtmMessage(enabled: Boolean, participant: Participant, onComplete: (Boolean) -> Unit) {
        ...
        val json = if (enabled)
            ...
            gson.toJson(RtmDataPacket(payload = gson.toJson(
                CancelHandRtmMessage(
                    topic = topic,
                    hands = identities
                )
            ), signature = "", sendTimestamp = System.currentTimeMillis(), uuid = java.util.UUID.randomUUID().toString()))

        send(topic = topic, payload = json, encrypt = false, onComplete = onComplete)
    }

    /**
     * Sends a data payload through the RTM (Real-Time Messaging) channel with optional encryption.
     */
    private fun send(topic: String, payload: String, encrypt: Boolean, onComplete: (Boolean) -> Unit, identities: List<Participant.Identity>? = null) {
        scope.launch(Dispatchers.IO) {
            val success = try {
                val data = if (encrypt) encryptor(payload.toByteArray(Charsets.UTF_8)) else payload
                if (data == null) {
