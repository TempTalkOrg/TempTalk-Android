 ...
 *
 * 布局前乐观返回 visible=true：VideoRenderer 只在确实需要展示时才进入 Compose 树，
 * 因此在首次 onGloballyPositioned 回调前假定可见是安全的。这避免了 addRenderer 时
 * 因 coordinates 为 null 而错误报告 disabled=true 给服务端。
 */
open class ComposeVisibility : VideoSinkVisibility() {
    ...
    override fun isVisible(): Boolean {
        ...
        val coords = coordinates ?: return true
        return (coords.isAttached &&
            coords.size.width != 0 &&
            coords.size.height != 0)
    }
