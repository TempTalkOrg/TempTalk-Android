...
class HandsUpManager(private val scope: CoroutineScope, private val context: Context, private val contactorCacheManager: ContactorCacheManager) {
        ...
        scope.launch(Dispatchers.IO) {
            runCatching {
                ...
                val list = sorted.map { id ->
                    val userName = contactorCacheManager.getDisplayNameById(id) ?: id
                    val userAvatar = contactorCacheManager.getAvatarByUid(context, id)
                        ?: contactorCacheManager.createAvatarByNameOrUid(context, userName, id)
