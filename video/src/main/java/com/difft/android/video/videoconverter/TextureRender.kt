 ...
 */
internal class TextureRender(flipX: Boolean) {
    ...
    private val mTriangleVerticesDataFlippedX = floatArrayOf(
        // X, Y, Z, U, V
        -1.0f, -1.0f, 0f, 1f, 0f,
        1.0f, -1.0f, 0f, 0f, 0f,
        -1.0f, 1.0f, 0f, 1f, 1f,
    ...
    init {
        val verticesData = if (flipX) mTriangleVerticesDataFlippedX else mTriangleVerticesData
    ...
    fun surfaceCreated() {
        ...
        if (mProgram == 0) {
            throw TranscodingException("failed creating program")
        }
        maPositionHandle = GLES20.glGetAttribLocation(mProgram, "aPosition")
        checkGlError("glGetAttribLocation aPosition")
        if (maPositionHandle == -1) {
            throw TranscodingException("Could not get attrib location for aPosition")
        }
        maTextureHandle = GLES20.glGetAttribLocation(mProgram, "aTextureCoord")
        checkGlError("glGetAttribLocation aTextureCoord")
        if (maTextureHandle == -1) {
            throw TranscodingException("Could not get attrib location for aTextureCoord")
        }

        muMVPMatrixHandle = GLES20.glGetUniformLocation(mProgram, "uMVPMatrix")
        checkGlError("glGetUniformLocation uMVPMatrix")
        if (muMVPMatrixHandle == -1) {
            throw TranscodingException("Could not get attrib location for uMVPMatrix")
        }

        muSTMatrixHandle = GLES20.glGetUniformLocation(mProgram, "uSTMatrix")
        checkGlError("glGetUniformLocation uSTMatrix")
        if (muSTMatrixHandle == -1) {
            throw TranscodingException("Could not get attrib location for uSTMatrix")
        }
    ...
    companion object {
        ...
        fun checkGlError(msg: String) {
            ...
            while (true) {
                ...
                L.e { TAG + msg + ": GLES20 error: 0x" + Integer.toHexString(error) }
                failed = true
            }
            if (failed) {
                throw TranscodingException("GLES20 error encountered (see log)")
            }
        }
    }
}
