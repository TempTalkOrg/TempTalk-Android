...
class RuleEditText : EditableTextView {
    ...
    private fun init(context: Context, attributes: AttributeSet) {
        context.withStyledAttributes(attributes, R.styleable.RuleEditText) {
            val ruleFlags = getInt(R.styleable.RuleEditText_rule, TYPE_MASK)
            rule = when (ruleFlags) {
                TYPE_MASK -> {
                    getString(R.styleable.RuleEditText_rule)?.let { RegexRule(Regex(it)) } ?: None
                }
                else -> {
                    //
                    var rules = UNKNOWN
