 ...
 * swallow logs + records a non-fatal Crashlytics breadcrumb (wrapped in [runCatching] so telemetry
 * failure can't itself crash). Every other throwable is forwarded to
    ...
    if (throwable is WCDBKeyUnavailableException) {
        ...
        runCatching {
            FirebaseCrashlytics.getInstance().apply {
                setCustomKey("db_key_failsoft_scope", scopeName)
                recordException(throwable) // non-fatal breadcrumb
            }
        }
    } else {
