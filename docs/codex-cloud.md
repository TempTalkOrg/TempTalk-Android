...
Setup also attempts to install Codex CLI 0.146.0 and `dynamic-workflow` 0.2.2. Those capabilities are optional: installation failures produce warnings but never fail core Android provisioning. On a cached-container resume, maintenance replaces an older pinned marketplace snapshot, reinstalls a mismatched plugin cache, retries provisioning, and verifies the installed plugin metadata and bundled runner before the agent starts. Detached runs keep a durable runner snapshot, so cache replacement does not remove an active supervisor or notifier executable; 0.2.2 retains the prior durable-state schema for later controls.

`dynamic-workflow` is installed from `pchalasani/claude-code-tools` at release commit `8c1d08e65bf71e0b1bb62d5ee9d1bc9cdd05442f`. Selecting this repository's setup command is explicit approval to install that pinned third-party plugin in the Cloud environment. Provisioning changes it only when this repository's explicit pin changes, verifies the marketplace checkout's exact Git revision, and refuses to replace the shared marketplace while another plugin from it is installed. Start a new Cloud task after installing, repairing, or changing the plugin pin so Codex reloads the skill catalog.
...
- If `dynamic-workflow` is missing, has the wrong version, or is not sourced from the pinned commit, let maintenance repair it on the next cached task or reset the environment cache. Maintenance stops safely if another plugin from `cctools-codex-plugins` is installed; migrate or remove that plugin before replacing the shared marketplace. For a manual repair, remove `dynamic-workflow@cctools-codex-plugins`, remove the marketplace, add `pchalasani/claude-code-tools` at ref `8c1d08e65bf71e0b1bb62d5ee9d1bc9cdd05442f`, reinstall `dynamic-workflow@cctools-codex-plugins`, and start a new task.
