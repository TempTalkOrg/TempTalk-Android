...
class SqlCipherErrorHandler(private val databaseName: String) : DatabaseErrorHandler {
    ...
    override fun onCorruption(db: SQLiteDatabase, message: String?) {
        ...
        lines = listOf("Database '$databaseName' corrupted! Message: $message. Diagnostics results:\n") + lines

        L.e { "Database '$databaseName' corrupted! Message: $message. Diagnostics results:\n ${result.logs}" }
    ...
    companion object {
        private val TAG = L.tag(SqlCipherErrorHandler::class.java)
    }
}
