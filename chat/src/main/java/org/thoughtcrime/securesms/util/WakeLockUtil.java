...
public class WakeLockUtil {
    ...
    private static final String TAG = L.INSTANCE.tag(WakeLockUtil.class);
    ...
    public static void release(@Nullable WakeLock wakeLock, @NonNull String tag) {
        ...
        try {
            if (wakeLock == null) {
                String finalTag = tag;
                L.d(() -> "Wakelock was null. Skipping. Tag: " + finalTag);
            } else if (wakeLock.isHeld()) {
                wakeLock.release();
            } else {
                String finalTag1 = tag;
                L.d(() -> "Wakelock wasn't held at time of release: " + finalTag1);
            }
        } catch (Exception e) {
