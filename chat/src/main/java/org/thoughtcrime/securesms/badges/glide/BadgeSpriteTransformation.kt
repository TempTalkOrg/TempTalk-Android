...
class BadgeSpriteTransformation(
  ...
  override fun transform(pool: BitmapPool, toTransform: Bitmap, outWidth: Int, outHeight: Int): Bitmap {
    ...
    if (inBounds.width() != outWidth || inBounds.height() != outHeight) {
      L.d { "Bitmap size mismatch, performing filtered scale. (Wanted $outWidth x $outHeight but got ${inBounds.width()} x ${inBounds.height()})" }
      ...
      L.d { "Bitmap size match, performing direct draw. ($outWidth x $outHeight)" }
  ...
  companion object {
    ...
    private val TAG = L.tag(BadgeSpriteTransformation::class.java)
