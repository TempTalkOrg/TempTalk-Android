...
public class JobManager implements ConstraintObserver.Notifier {
    ...
    private static final String TAG = L.INSTANCE.tag(JobManager.class);
        ...
        executor.execute(() -> {
            synchronized (this) {
                try {
                    ...
                    L.e(() -> "Failed to initialize JobManager: " + e.getMessage());
                    // 即使初始化失败，也标记为已初始化，避免无限等待
                    initialized = true;
                    notifyAll();
                }
            }
    ...
    private void waitUntilInitialized() {
        if (!initialized) {
            L.i(() -> "Waiting for initialization...");
            synchronized (this) {
                while (!initialized) {
                    Util.wait(this, 0);
                }
            }
            L.i(() -> "Initialization complete.");
        }
    }
