...
class JobController {
    ...
    private static final String TAG = L.INSTANCE.tag(JobController.class);
    ...
    synchronized void submitNewJobChain(@NonNull List<List<Job>> chain) {
        ...
        if (chain.isEmpty()) {
            L.w(() -> "Tried to submit an empty job chain. Skipping.");
            return;
        }
        ...
        if (chainExceedsMaximumInstances(chain)) {
            ...
            L.w(() -> JobLogger.format(solo, "Already at the max instance count. Factory limit: " + solo.getParameters().getMaxInstancesForFactory() + ", Queue limit: " + solo.getParameters().getMaxInstancesForQueue() + ". Skipping."));
            return;
        }
    ...
    synchronized void submitJobWithExistingDependencies(@NonNull Job job, @NonNull Collection<String> dependsOn, @Nullable String dependsOnQueue) {
        ...
        if (chainExceedsMaximumInstances(chain)) {
            ...
            L.w(() -> JobLogger.format(job, "Already at the max instance count. Factory limit: " + job.getParameters().getMaxInstancesForFactory() + ", Queue limit: " + job.getParameters().getMaxInstancesForQueue() + ". Skipping."));
            return;
        }
        ...
        if (jobTracker.haveAnyFailed(allDependsOn)) {
            L.w(() -> "This job depends on a job that failed! Failing this job immediately.");
    ...
    private @Nullable Job createJob(@NonNull JobSpec jobSpec, @NonNull List<ConstraintSpec> constraintSpecs) {
        ...
        try {
            ...
            L.w(() -> "Failed to instantiate job! Failing it and its dependencies without calling Job#onFailure. Crash imminent.");
            ...
            L.e(() -> "Failed " + failIds.size() + " dependent jobs.");
