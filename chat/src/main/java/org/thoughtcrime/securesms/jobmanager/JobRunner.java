...
class JobRunner extends Thread {
    ...
    private static final String TAG = L.INSTANCE.tag(JobRunner.class);
    ...
    private Job.Result run(@NonNull Job job) {
        ...
        L.i(() -> JobLogger.format(job, String.valueOf(id), "Running job."));

        if (isJobExpired(job)) {
            L.w(() -> JobLogger.format(job, String.valueOf(id), "Failing after surpassing its lifespan."));
        ...
        try {
            ...
            if (job.isCanceled()) {
                L.w(() -> JobLogger.format(job, String.valueOf(id), "Failing because the job was canceled."));
                result = Job.Result.failure();
            }
        } catch (Exception e) {
            L.w(() -> JobLogger.format(job, String.valueOf(id), "Failing due to an unexpected exception.") + e);
            return Job.Result.failure();
        } finally {
            if (wakeLock != null) {
        ...
        if (result.isRetry() &&
            ...
            L.w(() -> JobLogger.format(job, String.valueOf(id), "Failing after surpassing its max number of attempts."));
            return Job.Result.failure();
        }
    ...
    private void printResult(@NonNull Job job, @NonNull Job.Result result, long runStartTime) {
        if (result.getException() != null) {
            L.e(() -> JobLogger.format(job, String.valueOf(id), "Job failed with a fatal exception. Crash imminent."));
        } else if (result.isFailure()) {
            L.w(() -> JobLogger.format(job, String.valueOf(id), "Job failed."));
        } else {
            L.i(() -> JobLogger.format(job, String.valueOf(id), "Job finished with result " + result + " in " + (System.currentTimeMillis() - runStartTime) + " ms."));
        }
    }
}
