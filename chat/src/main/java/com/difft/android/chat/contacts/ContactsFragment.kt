...
class ContactsFragment : Fragment(), DualPaneSelectionListener {
    ...
    private fun initPage() {
        binding.viewpager.apply {
            adapter = object : FragmentStateAdapter(this@ContactsFragment) {
                private val fragmentClasses: List<Class<out Fragment>> = listOf(
    ...
    override fun updateDualPaneSelection(selectedId: String?) {
        // Forward to all child fragments that implement DualPaneSelectionListener
        childFragmentManager.fragments.forEach { fragment ->
            (fragment as? DualPaneSelectionListener)?.updateDualPaneSelection(selectedId)
        }
    }
