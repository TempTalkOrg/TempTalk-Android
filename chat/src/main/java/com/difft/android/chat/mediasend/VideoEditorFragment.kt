...
import com.difft.android.chat.scribbles.VideoEditorPlayButtonLayout
import com.difft.android.chat.util.Throttler
import com.difft.android.chat.util.visible
import com.difft.android.chat.video.VideoPlayer
import com.difft.android.chat.video.VideoPlayer.PlayerCallback
import com.difft.android.chat.video.videoconverter.VideoThumbnailsRangeSelectorView
import com.difft.android.chat.video.videoconverter.VideoThumbnailsRangeSelectorView.PositionDragListener
import java.io.IOException
import kotlin.time.Duration.Companion.microseconds

class VideoEditorFragment : androidx.fragment.app.Fragment(), PositionDragListener, MediaSendPageFragment {
    ...
    private val updatePosition = object : Runnable {
        override fun run() {
            if (MediaConstraints.isVideoTranscodeAvailable()) {
                val playbackPositionUs = player.playbackPositionUs + startTimeUs
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Bundle may lose the Parcelable Uri after process death; null-check instead of !!.
        val args = arguments
        val parcelableUri: Uri? = if (Build.VERSION.SDK_INT >= 33) {
            args?.getParcelable(KEY_URI, Uri::class.java)
        } else {
            @Suppress("DEPRECATION")
            args?.getParcelable(KEY_URI)
        }
        if (args == null || parcelableUri == null) {
            L.w { "[VideoEditorFragment] uri arg missing after restore, finishing" }
            ToastUtil.showLong(R.string.media_editor_unavailable)
            activity?.finish()
            return
        }
        uri = parcelableUri
        isVideoGif = args.getBoolean(KEY_IS_VIDEO_GIF, false)
        maxSend = args.getLong(KEY_MAX_SEND, 0L)

        player.setWindow(requireActivity().window)
        player.setVideoSource(uri, isVideoGif, TAG)

        hud.visible = !isVideoGif
