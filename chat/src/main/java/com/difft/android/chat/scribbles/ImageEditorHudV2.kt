...
import com.difft.android.chat.scribbles.HSVColorSlider.setColor
...
class ImageEditorHudV2 @JvmOverloads constructor(
    ...
    // Lazy injection: this class is XML-inflated (not @AndroidEntryPoint). Resolves to the
    // Application's UserManager via Hilt's EntryPointAccessors, which unwraps any
    // ContextThemeWrapper / FragmentContextWrapper chain.
    private val userManager: UserManager by lazy {
        context.globalHiltServices().userManager
    }
...
//    private val stickerButton: View = findViewById(R.id.image_editor_hud_sticker_button)
    ...
    private val selectableSet: Set<View> = setOf(drawButton, textButton, blurButton)
    ...
    private val drawButtonRow: Set<View> = setOf(cancelButton, doneButton, drawButton, textButton, blurButton)
...
//    stickerButton.setOnClickListener { setMode(Mode.INSERT_STICKER) }
        ...
        widthSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                ...
                when (currentMode) {
                    ...
                    Mode.HIGHLIGHT -> userManager.update { imageEditorHighlighterPercentage = progress }
                    else -> Unit
                }
            }
    ...
    private fun presentModeDraw() {
        ...
        widthSeekBar.progress = userManager.getUserData()?.imageEditorMarkerPercentage ?: 0
        listener?.onColorChange(getActiveColor())
        updateColorIndicator()
        animateModeChange(
    ...
    private fun presentModeHighlight() {
        ...
        widthSeekBar.progress = userManager.getUserData()?.imageEditorHighlighterPercentage ?: 0
        listener?.onColorChange(getActiveColor())
        updateColorIndicator()
        animateModeChange(
            inSet = drawButtonRow + drawTools,
    ...
    private fun presentModeBlur() {
        ...
        widthSeekBar.progress = userManager.getUserData()?.imageEditorBlurPercentage ?: 0
        listener?.onColorChange(getActiveColor())
        updateColorIndicator()
        animateModeChange(
    ...
    companion object {
        ...
        // Inlined from deleted ImageEditorValues — pure constants, no persistence.
        // Values preserved exactly (see design §3.4).
        private val MARKER_WIDTH_RANGE: Pair<Float, Float> = 0.01f to 0.05f
        private val HIGHLIGHTER_WIDTH_RANGE: Pair<Float, Float> = 0.03f to 0.08f
        private val BLUR_WIDTH_RANGE: Pair<Float, Float> = 0.052f to 0.092f

        private val DRAW_WIDTH_BOUNDARIES: Map<Mode, Pair<Float, Float>> = mapOf(
            Mode.DRAW to MARKER_WIDTH_RANGE,
            Mode.HIGHLIGHT to HIGHLIGHTER_WIDTH_RANGE,
            Mode.BLUR to BLUR_WIDTH_RANGE
        )
